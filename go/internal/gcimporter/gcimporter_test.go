...
func TestImportTestdata(t *testing.T) {
	...
	if pkg := testPath(t, "./testdata/exports", tmpdir); pkg != nil {
		...
		for _, want := range []string{"github.com/faiface/generics/go/ast", "github.com/faiface/generics/go/token"} {
...
var importedObjectTests = []struct {
	...
	{"github.com/faiface/generics/go/ast.ObjKind", "type ObjKind int"},
	{"github.com/faiface/generics/go/types.Qualifier", "type Qualifier func(*Package) string"},
	{"github.com/faiface/generics/go/types.Comparable", "func Comparable(T Type) bool"},
	{"math.Pi", "const Pi untyped float"},
	{"math.Sin", "func Sin(x float64) float64"},
	{"github.com/faiface/generics/go/ast.NotNilFilter", "func NotNilFilter(_ string, v reflect.Value) bool"},
	{"github.com/faiface/generics/go/internal/gcimporter.BImportData", "func BImportData(fset *go/token.FileSet, imports map[string]*go/types.Package, data []byte, path string) (_ int, pkg *go/types.Package, err error)"},
	...
	{"github.com/faiface/generics/go/ast.Node", "type Node interface{End() go/token.Pos; Pos() go/token.Pos}"},
	{"github.com/faiface/generics/go/types.Type", "type Type interface{String() string; Underlying() Type}"},
}
...
func TestIssue13898(t *testing.T) {
	...
	fset := token.NewFileSet()
	imports := make(map[string]*types.Package)
	_, err := Import(fset, imports, "github.com/faiface/generics/go/internal/gcimporter", ".", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	for path, pkg := range imports {
		if path == "github.com/faiface/generics/go/types" {
			goTypesPkg = pkg
			break
		}
	}
	if goTypesPkg == nil {
		t.Fatal("github.com/faiface/generics/go/types not found")
	}
	...
	if !ok {
		t.Fatalf("github.com/faiface/generics/go/types.Object type is %v; wanted named type", typ)
	}

	// lookup go/types.Object.Pkg method
	m, index, indirect := types.LookupFieldOrMethod(typ, false, nil, "Pkg")
	if m == nil {
		t.Fatalf("github.com/faiface/generics/go/types.Object.Pkg not found (index = %v, indirect = %v)", index, indirect)
	}

	// the method must belong to go/types
	if m.Pkg().Path() != "github.com/faiface/generics/go/types" {
