...
func (check *Checker) builtin(x *operand, call *ast.CallExpr, id builtinId) (_ bool) {
	...
	case _Append:
		...
		if nargs == 2 && call.Ellipsis.IsValid() && x.assignableTo(nil, check.conf, NewSlice(universeByte), nil) {
			arg(x, 1)
			if x.mode == invalid {
				return
			}
		...
		sig.variadic = true
		check.arguments(nil, x, call, sig, func(x *operand, i int) {
	...
	case _Delete:
		...
		if !x.assignableTo(nil, check.conf, m.key, nil) {
	...
	case _New:
		// new(T)
		// (no argument evaluated yet)
		T := check.typ(nil, call.Args[0], false)
		if T == Typ[Invalid] {
			return
		}
	...
	case _Panic:
		// panic(x)
		check.assignment(nil, x, &emptyInterface, "argument to panic")
		if x.mode == invalid {
			return
		}
	...
	case _Print, _Println:
		...
		if nargs > 0 {
			...
			for i := 0; i < nargs; i++ {
				...
				check.assignment(nil, x, nil, "argument to "+predeclaredFuncs[id].name)
				if x.mode == invalid {
	...
	case _Alignof:
		...
		check.assignment(nil, x, nil, "argument to unsafe.Alignof")
		if x.mode == invalid {
			return
		}
	...
	case _Offsetof:
		...
		sel := selx.Sel.Name
		obj, index, indirect, _ := LookupFieldOrMethod(base, false, check.pkg, sel)
		switch obj.(type) {
		case nil:
	...
	case _Sizeof:
		...
		check.assignment(nil, x, nil, "argument to unsafe.Sizeof")
		if x.mode == invalid {
			return
		}
