...
type declInfo struct {
	...
	typeParams []*ast.TypeParam // list of type parameters, or nil
			...
			case *ast.GenDecl:
				...
				for iota, spec := range d.Specs {
					...
					case *ast.TypeSpec:
						...
						check.declarePkgObj(s.Name, obj, &declInfo{file: fileScope, typeParams: s.Params, typ: s.Type, alias: s.Assign.IsValid()})
			...
			case *ast.FuncDecl:
				...
				if d.Recv == nil {
					...
					if list := d.Recv.List; len(list) > 0 {
						...
						if call, ok := typ.(*ast.CallExpr); ok { // receiver is a generic type
							typ = call.Fun
						}
