...
type Signature struct {
	...
	typeParams []*TypeParam // a list of type parameters (unnamed or not); nil if the function isn't generic
	recv       *Var         // nil if not a method
	unnamed    []*TypeParam // unnamed generic type parameters
...
func NewSignature(recv *Var, params, results *Tuple, variadic bool) *Signature {
	...
	return &Signature{nil, nil, recv, nil, params, results, variadic}
}

// Generics returns a list of all generic type parameters of the signature.
...
// Unnamed returns a list of all unnamed generic type parameters of the signature.
func (s *Signature) Unnamed() []*TypeParam { return s.unnamed }
...
type Named struct {
	...
	params     []*TypeParam // a list of type parameters; nil if type is not generic
...
func (t *Named) NumParams() int         { return len(t.params) }
func (t *Named) Param(i int) *TypeParam { return t.params[i] }
...
// An Instance is a generic type with its type parameters filled with arguments.
...
func NewInstance(named *Named, args []Type) *Instance {
	assert(len(named.params) == len(args))
	return &Instance{
		named: named,
		args:  args,
	}
...
func (t *Instance) Named() *Named {
	return t.named
}

func (t *Instance) NumArgs() int {
	return len(t.args)
}

func (t *Instance) Arg(i int) Type {
	return t.args[i]
}

func (t *Instance) Mapping() map[*TypeParam]Type {
	assert(t.named.NumParams() == len(t.args))
	mapping := make(map[*TypeParam]Type)
	for i := 0; i < t.named.NumParams(); i++ {
		mapping[t.named.Param(i)] = t.args[i]
	}
	return mapping
}
...
const (
	RestrictionEq = 1 << iota
	RestrictionOrd
	RestrictionNum
)

// A TypeParam represents a generic type parameter from a function signature.
type TypeParam struct {
	obj         *TypeName
	restriction Restriction
}

// NewTypeParam returns a new generic type with the specified type name.
func NewTypeParam(obj *TypeName) *TypeParam {
	return &TypeParam{obj: obj}
}
...
func (t *TypeParam) Restriction() Restriction {
...
func (t *Instance) Underlying() Type {
	...
	t.underlying = mapType(t.Mapping(), t.named.Underlying(), make(map[Type]Type))
	return t.underlying
}
