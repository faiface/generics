...
//
func (check *Checker) ident(scope *Scope, x *operand, e *ast.Ident, def *Named, path []*TypeName) {
	x.mode = invalid
	x.expr = e

	scope, obj := scope.LookupParent(e.Name, check.pos)
	if obj == nil {
		if e.Name == "_" {
...
//
func (check *Checker) typExpr(scope *Scope, e ast.Expr, def *Named, path []*TypeName, uninstantiatedOk bool) (T Type) {
	if trace {
	...
	T = check.typExprInternal(scope, e, def, path, uninstantiatedOk)
	assert(isTyped(T))
...
func (check *Checker) typ(scope *Scope, e ast.Expr, uninstantiatedOk bool) Type {
	return check.typExpr(scope, e, nil, nil, uninstantiatedOk)
}

// funcType type-checks a function or method type.
func (check *Checker) funcType(generic *Scope, sig *Signature, recvPar *ast.FieldList, ftyp *ast.FuncType) {
	...
	recvList, _ := check.collectParams(generic, scope, recvPar, false)
	params, variadic := check.collectParams(generic, scope, ftyp.Params, true)
	results, _ := check.collectParams(generic, scope, ftyp.Results, false)
...
//
func (check *Checker) typExprInternal(scope *Scope, e ast.Expr, def *Named, path []*TypeName, uninstantiatedOk bool) Type {
	switch e := e.(type) {
	case *ast.BadExpr:
	...
	case *ast.Ident:
		var x operand
		if scope == nil {
			check.ident(check.scope, &x, e, def, path)
		} else {
			check.ident(scope, &x, e, def, path)
		}

		switch x.mode {
		case typexpr:
			// generic types not allowed without type arguments
			typ := x.typ
			if named, ok := x.typ.(*Named); !uninstantiatedOk && ok && named.NumParams() > 0 {
				check.errorf(e.Pos(), "%s used without type arguments", e.Name)
				typ = Typ[Invalid]
			}
	...
	case *ast.CallExpr:
		named, isNamed := check.typ(scope, e.Fun, true).(*Named)
		if !isNamed || named.NumParams() == 0 {
			check.errorf(e.Pos(), "not a generic type")
			return Typ[Invalid]
		}

		if named.NumParams() != len(e.Args) {
			check.errorf(e.Pos(), "type %s requires %d generic arguments, got %d",
				named.String(), named.NumParams(), len(e.Args))
			return Typ[Invalid]
		}

		var args []Type
		for _, arg := range e.Args {
			args = append(args, check.typ(scope, arg, false))
		}

		inst := NewInstance(named, args)
		def.setUnderlying(inst)
		if m := check.Info.GenericInstances; m != nil {
			check.Info.GenericInstances[e] = &GenericInstance{
	...
	case *ast.ParenExpr:
		return check.typExpr(scope, e.X, def, path, false)
	...
	case *ast.ArrayType:
		if e.Len != nil {
			...
			typ.elem = check.typExpr(scope, e.Elt, nil, path, false)
			return typ

		} else {
			typ := new(Slice)
			def.setUnderlying(typ)
			typ.elem = check.typ(scope, e.Elt, false)
			return typ
		}
	...
	case *ast.StructType:
		typ := new(Struct)
		def.setUnderlying(typ)
		check.structType(scope, typ, e, path)
		return typ

	case *ast.StarExpr:
		typ := new(Pointer)
		def.setUnderlying(typ)
		typ.base = check.typ(scope, e.X, false)
		return typ

	case *ast.FuncType:
		typ := new(Signature)
		def.setUnderlying(typ)
		check.funcType(scope, typ, nil, e)
		return typ

	case *ast.InterfaceType:
		typ := new(Interface)
		def.setUnderlying(typ)
		check.interfaceType(scope, typ, e, def, path)
		return typ
	...
	case *ast.MapType:
		...
		typ.key = check.typ(scope, e.Key, false)
		typ.elem = check.typ(scope, e.Value, false)
	...
	case *ast.TypeParam:
		...
		typ.obj = &TypeName{object{nil, e.Name.Pos(), check.pkg, e.Name.Name, typ, 0, token.NoPos}}
		typ.restriction = Restriction(e.Restriction)
		check.declare(scope, e.Name, typ.obj, e.Name.Pos())
		return typ
...
func (check *Checker) collectParams(generic, scope *Scope, list *ast.FieldList, variadicOk bool) (params []*Var, variadic bool) {
	if list == nil {
		return
	}
	...
	for i, field := range list.List {
		...
		typ := check.typ(generic, ftype, false)
...
func (check *Checker) interfaceType(scope *Scope, iface *Interface, ityp *ast.InterfaceType, def *Named, path []*TypeName) {
	...
	if def != nil {
		if def.NumParams() > 0 {
			...
			for i := 0; i < def.NumParams(); i++ {
				args = append(args, def.Param(i))
			}
			recvTyp = NewInstance(def, args)
		} else {
			recvTyp = def
		}
	}
	...
	for _, e := range embedded {
		pos := e.Pos()
		typ := check.typExpr(scope, e, nil, path, false)
	...
	for i, m := range iface.methods {
		expr := signatures[i]
		typ := check.typ(scope, expr, false)
		sig, _ := typ.(*Signature)
		if sig == nil {
			if typ != Typ[Invalid] {
...
func (check *Checker) structType(scope *Scope, styp *Struct, e *ast.StructType, path []*TypeName) {
	list := e.Fields
	if list == nil {
		return
	}
	...
	for _, f := range list.List {
		typ = check.typExpr(scope, f.Type, nil, path, false)
		tag = check.tag(f.Tag)
		if len(f.Names) > 0 {
			// named fields
