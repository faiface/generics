...
func (check *Checker) convertUntyped(x *operand, target Type) {
	...
	case *Pointer, *Signature, *Slice, *Map, *Chan:
		...
		target = Typ[UntypedNil]
	case *TypeParam:
		if t.Restriction()&RestrictionNum == 0 {
			goto Error
		}
		return
	default:
...
func (check *Checker) comparison(x, y *operand, op token.Token) {
	...
	err := ""
	if x.assignableTo(nil, check.conf, y.typ, nil) || y.assignableTo(nil, check.conf, x.typ, nil) {
		defined := false
		switch op {
...
func (check *Checker) indexedElts(elts []ast.Expr, typ Type, length int64) int64 {
	...
	for _, e := range elts {
		...
		check.assignment(nil, &x, typ, "array or slice literal")
	}
	return max
}
...
func (check *Checker) exprInternal(x *operand, e ast.Expr, hint Type) exprKind {
	...
	case *ast.Ident:
		check.ident(check.scope, x, e, nil, nil)
	...
	case *ast.FuncLit:
		if sig, ok := check.typ(nil, e.Type, false).(*Signature); ok {
	...
	case *ast.CompositeLit:
		...
		case e.Type != nil:
			...
			if atyp, _ := e.Type.(*ast.ArrayType); atyp != nil && atyp.Len != nil {
				if ellip, _ := atyp.Len.(*ast.Ellipsis); ellip != nil && ellip.Elt == nil {
					...
					typ = &Array{len: -1, elem: check.typ(nil, atyp.Elt, false)}
					base = typ
					break
				}
			}
			typ = check.typ(nil, e.Type, false)
			base = typ
		...
		case *Struct:
			...
			if _, ok := e.Elts[0].(*ast.KeyValueExpr); ok {
				...
				for _, e := range e.Elts {
					...
					etyp := fld.typ
					check.assignment(nil, x, etyp, "struct literal")
				}
			} else {
		...
		case *Map:
			...
			for _, e := range e.Elts {
				...
				check.assignment(nil, x, utyp.key, "map literal")
				if x.mode == invalid {
					continue
				}
				if x.mode == constant_ {
				...
				check.assignment(nil, x, utyp.elem, "map literal")
			}
	...
	case *ast.IndexExpr:
		...
		case *Map:
			...
			check.assignment(nil, &key, typ.key, "map index")
			if x.mode == invalid {
				goto Error
			}
			x.mode = mapindex
