...
func (x *operand) assignableTo(mapping map[*TypeParam]Type, conf *Config, T Type, reason *string) bool {
	...
	if identical(mapping, V, T, true, nil) {
		return true
	}
	...
	if identical(mapping, Vu, Tu, true, nil) && (!isNamed(V) || !isNamed(T)) {
		return true
	}
